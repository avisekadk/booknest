        ...
        <>
              ...
              <tbody>
                {currentUsers.map((user, index) => {
                  const returnedBooksCount =
                    user?.borrowedBooks?.filter((book) => book.returned).length ||
                    0;
                  const rowNumber = indexOfFirstUser + index + 1; // Calculate the correct row number
          ...
          {totalPages > 1 && (
            <nav className="mt-8 flex justify-center">
              <ul className="flex list-none rounded-lg shadow-md bg-white p-2">
                <li>
                  <button
                    onClick={() => paginate(currentPage - 1)}
                    disabled={currentPage === 1}
                    className={`px-4 py-2 mx-1 rounded-lg focus:outline-none transition duration-150 ease-in-out
                      ${currentPage === 1
